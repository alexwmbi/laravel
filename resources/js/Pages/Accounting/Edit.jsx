...
export default function Edit({ auth, accounting, detailAccountings = [], success }) {
  ...
  // 🔧 campi allineati ai nomi che arrivano dal backend
 const { data, setData, put, processing, errors } = useForm({
  ...
  // 🔧 righe dettagli: il controller passa chiavi in minuscolo
        ...
        <span className={`inline-block rounded px-2 py-0.5 text-xs ${ACCOUNTING_STATUS_CLASS_MAP[data.Stato]}`}>
          {ACCOUNTING_STATUS_TEXT_MAP[data.Stato]}
            ...
            <div className="mt-8 flex items-center justify-end gap-3">
              <Link href={route("accounting.index")} className="rounded-md border bg-white px-4 py-2 text-sm shadow-sm hover:bg-gray-50">
                Annulla
              </Link>
              <button
                type="submit"
